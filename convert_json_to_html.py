...
/* ---- stage bar ---- */
.stagebar{display:flex;gap:2px;width:100%}
.seg{border:0;padding:.7rem .4rem .6rem;font-family:var(--sans);cursor:pointer;text-align:left;
  display:flex;flex-direction:column;gap:.1rem;overflow:hidden;min-width:0;color:var(--ink)}
.seg:hover{outline:2px solid var(--ink);outline-offset:-2px}
.seg[aria-pressed="true"]{outline:3px solid var(--flag);outline-offset:-3px}
.seg-n{font-size:1.05rem;font-weight:600;line-height:1}
.seg-l{font-size:.7rem;line-height:1.15;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.seg.s1{background:var(--b1)}
.seg.s2{background:var(--b2)}
.seg.s3{background:var(--b3)}
.seg.s4{background:var(--b3)}
.seg.s5{background:var(--b4);color:var(--paper)}
.seg.s6{background:var(--b5);color:var(--paper)}
.seg.sx{background:var(--dead);color:var(--ink-soft)}
@media(max-width:700px){.seg-l{display:none}.seg{padding:.7rem .2rem;align-items:center}}
...
<section>
  ...
  <p class="note">Most bills die in the chamber they were introduced in. Select a stage to filter the register.</p>
  <hr class="rule">
...
(function(){
  ...
  function apply(){
    ...
    var shown = cards.filter(function(c){
      if (st === 'US' ? c.dataset.stateCode !== 'US' : (st !== 'all' && c.dataset.state !== st)) return false;
      if (stg !== 'all' && c.dataset.status !== stg) return false;
      if (q && c._text.indexOf(q) === -1) return false;
      return true;
    });
    ...
    if (q) bits.push('matching &ldquo;' + q.replace(/[<>&]/g,'') + '&rdquo;');
    ...
    // reflect current selection on the map and stage bar
    Array.prototype.forEach.call(document.querySelectorAll('.tile.has-bills, .fedtile'), function(t){
      t.setAttribute('aria-pressed', String(t.dataset.state === st));
    });
    Array.prototype.forEach.call(stagebar.querySelectorAll('.seg'), function(s){
  ...
  map.addEventListener('click', function(e){
    ...
    if (t) pickState(t.dataset.state);
  });
...
def build_stage_bar(stage_counts, total):
    """Render the proportional stage bar. Only stages with bills are shown."""
    segs = []
    seen = set()
    for stage, cls in STAGE_SEQUENCE:
        ...
        if not n:
            continue
        pct = max(n / total * 100, 2.2) if total else 1
        segs.append(
            '<button type="button" class="seg %s" data-status="%s" aria-pressed="false" '
            'style="flex:%f" aria-label="%s, %d bills">'
            '<span class="seg-n">%d</span><span class="seg-l">%s</span></button>'
            % (cls, escape_html(stage), pct, escape_html(stage), n, n, escape_html(stage)))
    # Any status the API returns that is not in STAGE_SEQUENCE still gets a segment.
    for stage, n in sorted(stage_counts.items()):
        if stage in seen or not n:
            continue
        pct = max(n / total * 100, 2.2) if total else 1
        segs.append(
            '<button type="button" class="seg sx" data-status="%s" aria-pressed="false" '
            'style="flex:%f" aria-label="%s, %d bills">'
            '<span class="seg-n">%d</span><span class="seg-l">%s</span></button>'
            % (escape_html(stage), pct, escape_html(stage), n, n, escape_html(stage)))
    return '\n'.join(segs)
